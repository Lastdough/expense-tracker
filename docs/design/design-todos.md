...
- [x] **Print stylesheet** — receipt preview uses screen colors. Real print needs: page-break rules between sections, header repeating on each page, no shadows/borders that don't print, "Page N of M" footer. *(Closed by `feature/v2.3-receipt-redesign` — `receiptHtml.ts` ships the v2.3 Simple layout with `@page A4 portrait`, repeating `thead` via `table-header-group`, `break-inside: avoid` on rows + critical blocks, monochrome print colors, and a `counter(page)/counter(pages)` "Page N of M" footer. PDF is still H.4.)*
