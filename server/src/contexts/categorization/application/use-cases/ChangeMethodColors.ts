...
export interface ChangeMethodColorsInput {
  readonly id: string;
  readonly bgColor: string;
  readonly textColor: string;
}

export class ChangeMethodColors {
  constructor(private readonly methods: IMethodRepository) {}

  async execute(input: ChangeMethodColorsInput): Promise<Result<Method, MethodNotFoundError>> {
    ...
    method.changeColors({ bgColor: input.bgColor, textColor: input.textColor });
